...
{
    if (value < 10.f)      return juce::String(value, 2) + " ms";
    else if (value < 100.f) return juce::String(value, 1) + " ms";
    else if (value < 1000.f) return juce::String(int(value)) + " ms";
...
juce::AudioProcessorValueTreeState::ParameterLayout Parameters::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(
        ...
        juce::NormalisableRange<float>{ 0.f, 90.f, 1.f },
        0.f,
        juce::AudioParameterFloatAttributes().withStringFromValueFunction(stringFromPercent)
    ));
    ...
    layout.add(std::make_unique<juce::AudioParameterChoice>(
        delayNoteParamID, "Delay Note",
        juce::StringArray{
            "1/32", "1/16 trip", "1/32 dot", "1/16", "1/8 trip", "1/16 dot",
...
void Parameters::smoothen() noexcept
{
    ...
    panningEqualPower(0.5f, panL, panR); // center input
